...
const EmailItem = (props: EmailItemProps) => {
    ...
    if (!props.isRead) {
      try {
        const res = await fetch(
          `http://localhost:3000/api/emails/${emailId}/read`,
          {
            method: "PATCH",
            headers: {
  ...
  return (
    <>
      <div
        className={ "email-item-tapin-flex-wrapper" + (props.showTapIn === false ? " no-tap-mode" : "")
        }
      >
        {props.showTapIn && (
        ...
        <div
          ...
          className={
            !props.isRead
              ? "email-content-date-flex"
              : "email-content-date-flex inactive"
          }
        >
